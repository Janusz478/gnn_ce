...
class JOBSyntheticBigDGL(torch.utils.data.Dataset):
    def __init__(self, name, split):
        self.path_job_graphs = "data/job-light-pickle/" + str.lower(name.split("-")[0]) + "/"
        self.path_synthetic_graphs = "data/synthetic-big-pickle/" + str.lower(name.split("-")[0]) + "/"
        self.split = split
        self.job_cardinalities_path = "data/job-light-pickle/team10_job_light_cardinalities.csv"
        self.synthetic_cardinalities_path = "data/synthetic-big-pickle/team10_synthetic_big_cardinalities.csv"
        self.graph_labels = []
        self.graph_lists = []
        self._prepare()
...
class JOBSyntheticBigDatasetDGL(torch.utils.data.Dataset):
    def __init__(self, name):
        t0 = time.time()

        self.train = JOBSyntheticBigDGL(name, 'train')
        self.test = JOBSyntheticBigDGL(name, 'test')
        self.val = JOBSyntheticBigDGL(name, 'val')
        print("Time taken: {:.4f}s".format(time.time() - t0))



class JOBSyntheticBigDataset(torch.utils.data.Dataset):
    def __init__(self, name):
        start = time.time()
